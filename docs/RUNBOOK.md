...
A per-build Software Bill of Materials is generated by **Syft** from the same
`devsecops-app:ci` image the container gate scans, in **CycloneDX JSON**, and uploaded
as the `sbom` build artifact. It is evidence, not a gate — Syft only fails on a real
tooling error, never on SBOM contents. See `docs/DECISIONS.md` ADR-014.

```bash
# The security-gates job already builds devsecops-app:ci; locally, build it first.
W=$(mktemp -d)                       # off-Drive scratch (FUSE can't bind-mount Drive)
cp -r app "$W"/ && mkdir -p "$W/sbom"
docker build -t devsecops-app:ci "$W/app"

# Syft → CycloneDX JSON (pinned official image, mirroring the scanner discipline)
docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v "$W:/src" \
  anchore/syft:v1.46.0 devsecops-app:ci \
  -o cyclonedx-json=/src/sbom/sbom.cyclonedx.json

# Sanity-check the output (bomFormat=CycloneDX, component count > 0)
python3 -c "import json; d=json.load(open('$W/sbom/sbom.cyclonedx.json')); \
  print(d['bomFormat'], d['specVersion'], len(d['components']), 'components')"
```
